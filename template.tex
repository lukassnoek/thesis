...
\begin{itemize}[label={}, itemsep=0pt, partopsep=3pt, topsep=-\parskip, parsep=0pt, leftmargin=1em]
  $if(ISBN)$\item ISBN: $ISBN$ $endif$
  $if(printing)$\item Printing: $printing$ $endif$
  $if(cover)$\item Cover art: $cover$ $endif$
\end{itemize}

$if(thesis_url)$
\noindent An online version of this thesis is available at $thesis_url$$if(license)$, licensed under a $license$.$endif$
$endif$
\endgroup
...
op $if(date)$$date$$else$[………]dag […date…] […month…] 20[..]$endif$, te 13.00 uur \\ }\par %
\vspace{\baselineskip}
{\large door}\par
\vspace{\baselineskip}
